...
<Mermaid 
  chart={`
graph LR
    A[Probabilistic Sampling] --> B[Deterministic Checks]
    B --> C[Zero-Knowledge Proofs]
    
    A1["Fast, 95% confidence<br/>Sample 10% outputs"] -.-> A
    B1["Hash matching<br/>Merkle trees"] -.-> B
    C1["Cryptographic proof<br/>Slow but absolute"] -.-> C
    
    style A fill:#ffebcc
    style B fill:#fff4cc
    style C fill:#ccf4ff
  `}
  id="verification-spectrum"
  title="Verification Solution Spectrum"
/>
...
sequenceDiagram
    ...
    participant EU_Node
    participant Non_EU_Node
    participant Blockchain
    
    User->>Orchestrator: Submit task with PII
    Orchestrator->>Orchestrator: Tag as GDPR-sensitive
    Orchestrator->>EU_Node: Route ONLY to EU jurisdiction
    Orchestrator-->>Non_EU_Node: Skip (compliance filter)
    EU_Node->>Blockchain: Anchor proof (NO PII data)
    
    Note over Blockchain: Only metadata on-chain:<br/>TaskID, NodeID, Timestamp
  `}
  id="gdpr-compliance-flow"
  title="GDPR Compliance Flow"
/>
