...
**Requirements:**
- OpenAI: prefix caching is automatic for prompts >1024 tokens
- Anthropic: use `cache_control: {"type": "ephemeral"}` on content blocks
- Minimum cacheable prefix: 1024 tokens (OpenAI), 1024 tokens (Anthropic)
- Cache TTL: 5 minutes (OpenAI), 5 minutes (Anthropic)
...
for label, model, cache, routing in scenarios:
    ...
    print(f"{label}")
    print(f"  Monthly cost: ${cost:,.0f}  (savings: {savings:.0f}%)")
    print()`}
/>
