...
const tutorials = defineCollection({
  ...
  schema: z.object({
    ...
    path: z.enum(['genai', 'llm-systems', 'langgraph', 'system-design', 'ai-literacy']).default('genai'),
    pathTitle: z.string().optional(),
    pathOrder: z.number().int().min(1).optional(),
    module: z.number().int().min(1),
    totalModules: z.number().int().min(1),
    roles: z.array(z.enum(['dev', 'qa', 'ba', 'pm', 'exec'])),
    tags: z.array(z.string()),
    relatedBlogs: z.array(z.string()).optional().default([]),
    date: z.date(),
